...
var all_questions = [
  ...
  {
    //Question 12
    question_string: "Complétez les trous dans le texte",
    tips:"Consigne : Pour compléter ce texte, vous devez faire glisser une des réponses ci dessous",
    dropText:"Acte",
    linkedFile:"",
    successMedia:"./media/Question11.gif",
    failMedia:"https://www.youtube.com/embed/TfLmXUrFFPw?autoplay=1",
...
Quiz.prototype.render = function(container) {
  ...
  function change_question() {
    ...
    if (self.questions[current_question_index].Type === "checkedList" || self.questions[current_question_index].Type === "holeList") {
  ...
  $('#next-question-button').click(function() {
    if (self.questions[current_question_index].user_choice_index === self.questions[current_question_index].correct_choice_index
     || (self.questions[current_question_index].Type === "joinedList" && validateJoinedList(self.questions[current_question_index].correctJoin))
      || (self.questions[current_question_index].Type === "holeList" && validateHoleList(self.questions[current_question_index].choices[self.questions[current_question_index].correct_choice_index]))){
...
// TODO : send and object instead of 50 params
var Question = function(question_string, correct_choice, wrong_choices, tips, linkedFile, type, successMedia, failMedia, correctJoin, dropText) {
  ...
  if (type === "checkedList") {
    ...
    this.choices = fillCheckedListChoices(this, wrong_choices, correct_choice);  
  } else if (type === "joinedList") {
    this.choices = fillJoinedListChoices(this, wrong_choices, correct_choice);
  } else if (type === "holeList") {
    this.choices = fillHoleListChoices(this, wrong_choices, correct_choice);
  }
}

var fillJoinedListChoices = function (self, wrong_choices, correct_choice) {
    ...
    for (var i = 0; i < number_of_choices; i++) {
      ...
      self.choices[i] = {"leftie":wrong_choices[lefties_choice_index], "rightie":correct_choice[righties_choice_index]};
...
var fillCheckedListChoices = function (self, wrong_choices, correct_choice) {
     // Fill in this.choices with the choices
    var number_of_choices = wrong_choices.length + 1;
    for (var i = 0; i < number_of_choices; i++) {
      if (i === self.correct_choice_index) {
        self.choices[i] = correct_choice;
      } else {
        // Randomly pick a wrong choice to put in this index
        var wrong_choice_index = Math.floor(Math.random(0, wrong_choices.length));
        self.choices[i] = wrong_choices[wrong_choice_index];
...
var fillHoleListChoices = function (self, wrong_choices, correct_choice) {
    // Random assign the correct choice an index
    self.correct_choice_index = Math.floor(Math.random() * wrong_choices.length + 1);
...
Question.prototype.render = function(container) {
  ...
  } else if (linkedFileContainer.children('table').length > 0) {
    linkedFileContainer.children('table').each(function() {
      $(this).remove();
    });    
  }
  ...
  } else if (this.Type === "holeList") {
    setChoiciesHoleText(container, self);
  }
}

var setChoiciesHoleText = function (container, me) {
  var dropZoneArea = $('<div>').attr("id", "dropZone")
                              .appendTo(container);
  var dropTextArea = $('<p>').attr("id", "dropText")
                            .text(me.dropText)
                            .appendTo(dropZoneArea);
  var dragZone = $('<div>').attr("id", "dragZone");
  var input = $('<input>').attr("id", "dropInput")
            .attr("name", "choices")
            .attr("class", "dropInput")
            .attr("ondragover", "allowDrop(event)")
            .attr("type", "text")
            .attr("value", "___________________")
            .appendTo(dropTextArea);
  for (var i = 0; i < me.choices.length; i++) {
    var containerTmp = $('<p>').attr('class', 'rowAnswerToDrag').appendTo(container)
    var answer = $('<div>').attr("id", "answer-"+i)
              .attr("class", "answer")
              .attr("draggable", "true")
  ...
  // Add a listener for the input in order to get the drop behaviour
  $('input[name=choices]').bind("drop", function(ev) {
...
$(document).ready(function() {
  ...
  for (var i = 0; i < all_questions.length; i++) {
    ...
    if (all_questions[i].Type === "checkedList") {
      question = new Question(all_questions[i].question_string, all_questions[i].choices.correct, all_questions[i].choices.wrong, all_questions[i].tips, all_questions[i].linkedFile, all_questions[i].Type, all_questions[i].successMedia, all_questions[i].failMedia, null, null);
    } else if (all_questions[i].Type === "joinedList") {
      question = new Question(all_questions[i].question_string, all_questions[i].choices.righties, all_questions[i].choices.lefties, all_questions[i].tips, all_questions[i].linkedFile, all_questions[i].Type, all_questions[i].successMedia, all_questions[i].failMedia, all_questions[i].choices.correctJoin, null);
    } else if (all_questions[i].Type === "holeList"){
      question = new Question(all_questions[i].question_string, all_questions[i].choices.correct, all_questions[i].choices.wrong, all_questions[i].tips, all_questions[i].linkedFile, all_questions[i].Type, all_questions[i].successMedia, all_questions[i].failMedia, null, all_questions[i].dropText);
    }
